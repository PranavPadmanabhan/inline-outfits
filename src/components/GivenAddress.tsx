...
function GivenAddress({Delete,AddressType,Name,Locality,City,PinNumber,PhoneNumber,onClick,onDelete,isSelected}:props) {
  return (
    <div onClick={onClick} className={`relative h-[100%] w-[100%] flex flex-col ${isSelected ? "border-[2px] border-[#1a25be]" : "border-[1px] border-[#00000013]"} justify-start items-center  rounded-xl py-2 box-border`}>
      <img onClick={onDelete} className="absolute top-2 right-2 h-[22px] w-[22px] mt-3 ml-44" src={Delete} alt="" />
